...
restart_server() {        # restart_server [VAR=valeur ...]
    "$SERVER" stop >/dev/null 2>&1 || true
    env "$@" "$SERVER" start >/dev/null
...
last_stats() { logs | grep -F "stats :" | tail -1 | sed 's/.*stats : //'; }
...
if [ "$MODE" != "lab" ]; then
    skipped "modele absent / SHA-256 / reprise" "exige LAB_AVATAR_* (mode lab)"
fi
...
if [ "$MODE" = "lab" ]; then
    ...
    forget_model; clear_log; restart_app
    if wait_log "SHA-256 incorrecte" 180 && ! has_log "page prete"; then
        installed="$(MSYS_NO_PATHCONV=1 "$ADB" shell "run-as $LAB_PACKAGE ls files/avatar/models 2>/dev/null" | tr -d '\r')"
...
if [ "$MODE" = "lab" ] && [ -x "$LINK" ]; then
    ...
    if [ -n "$played_after" ] && [ "$played_after" = "$played_before" ]; then
