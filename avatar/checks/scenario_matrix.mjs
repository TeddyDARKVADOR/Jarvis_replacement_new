...
/**
 * Les invariants d'une situation, sur ce que runCase a mesure. Renvoie
 * [[invariant, detail], ...] — vide quand tout tient. Exporte pour
 * eval_lab/js/face_runner.mjs, qui juge les memes situations et d'autres.
 */
export function violations(c, o) {
  const out = [];
  const fail = (kind, _c, detail) => out.push([kind, detail]);
...
export const MATRIX = { intents, states, speeches, gazes, urgencies, interruptions };
...
function main() {
  ...
  for (const c of cases) {
    const o = runCase(c);
    for (const [kind, detail] of violations(c, o)) fail(kind, c, detail);
...
// Lancee directement : la matrice. Importee (eval_lab) : seulement les fonctions.
